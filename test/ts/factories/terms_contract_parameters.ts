...
class TermsContractParameters {
  public static bitShiftLeft(target: BigNumber, numPlaces: number): BigNumber {
    ...
    const binaryTargetStringShifted =
      binaryTargetString + "0".repeat(numPlaces);
...
export class SimpleInterestParameters extends TermsContractParameters {
  public static pack(terms: SimpleInterestContractTerms): string {
    ...
    const termLengthShifted = TermsContractParameters.bitShiftLeft(
      terms.termLengthUnits,
      108
    );
...
export class CollateralizedSimpleInterestTermsParameters extends TermsContractParameters {
  public static pack(
    ...
    if (contractTerms) {
      const packedTermsParameters = SimpleInterestParameters.pack(
        contractTerms
      );
      return `${packedTermsParameters.substr(
        0,
        39
      )}${packedCollateralParameters.padStart(27, "0")}`;
    } else {
