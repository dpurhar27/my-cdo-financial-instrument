...
export async function submitMultiSigTransaction(
  ...
  const encodedTransaction = await contract.web3ContractInstance[
    methodName
...
async function submitRequisiteMultiSigConfirmations(
  ...
  for (let i = 1; i < 3; i++) {
    await multiSig.confirmTransaction.sendTransactionAsync(
      new BigNumber(transactionId),
      {
        from: accounts[i]
      }
    );
...
export async function multiSigExecuteAfterTimelock(
  ...
  const txHash = await multiSig.executeTransaction.sendTransactionAsync(
    transactionId,
    txData
  );
  ...
  if (!executedSuccessfully) {
    throw new Error(
      `Multisig transaction with ID #${transactionId} failed to execute.`
    );
  }
