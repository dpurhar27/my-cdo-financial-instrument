...
export class KittiesPausableContract extends BaseContract {
  public unpause = {
    async sendTransactionAsync(txData: TxData = {}): Promise<string> {
      const self = this as KittiesPausableContract;
  ...
  static async deployed(
  ...
  ): Promise<KittiesPausableContract> {
    ...
    return new KittiesPausableContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittiesPausable.json",
        "utf8"
      );
