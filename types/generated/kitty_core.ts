...
export class KittyCoreContract extends BaseContract {
  public supportsInterface = {
    async callAsync(
      _interfaceID: string,
      defaultBlock?: Web3.BlockParam
    ): Promise<boolean> {
      const self = this as KittyCoreContract;
  ...
  public newContractAddress = {
    async callAsync(defaultBlock?: Web3.BlockParam): Promise<string> {
      const self = this as KittyCoreContract;
      const result = await promisify<string>(
        self.web3ContractInstance.newContractAddress.call,
        self.web3ContractInstance
      )();
      return result;
    }
  ...
  public setNewAddress = {
    async sendTransactionAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
        txData,
        self.setNewAddress.estimateGasAsync.bind(self, _v2Address)
      );
      const txHash = await promisify<string>(
        self.web3ContractInstance.setNewAddress,
        self.web3ContractInstance
      )(_v2Address, txDataWithDefaults);
      return txHash;
    },
    async estimateGasAsync(
      _v2Address: string,
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.setNewAddress.estimateGas,
        self.web3ContractInstance
      )(_v2Address, txDataWithDefaults);
      return gas;
    },
    getABIEncodedTransactionData(
      ...
      const self = this as KittyCoreContract;
      const abiEncodedTransactionData = self.web3ContractInstance.setNewAddress.getData();
      return abiEncodedTransactionData;
    }
  };
  ...
  public getKitty = {
    ...
    > {
      ...
      >(self.web3ContractInstance.getKitty.call, self.web3ContractInstance)(
        _id
      );
      return result;
    }
  ...
  static async deployed(
    ...
    return new KittyCoreContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittyCore.json",
        "utf8"
      );
