...
export class DeprecatedERC721Contract extends BaseContract {
  ...
  public takeOwnership = {
    async sendTransactionAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
        txData,
        self.takeOwnership.estimateGasAsync.bind(self, _tokenId)
      );
      const txHash = await promisify<string>(
        self.web3ContractInstance.takeOwnership,
        self.web3ContractInstance
    ...
    async estimateGasAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.takeOwnership.estimateGas,
        self.web3ContractInstance
      )(_tokenId, txDataWithDefaults);
      return gas;
    },
    getABIEncodedTransactionData(
      ...
      const self = this as DeprecatedERC721Contract;
      const abiEncodedTransactionData = self.web3ContractInstance.takeOwnership.getData();
      return abiEncodedTransactionData;
    }
  };
  public safeTransferFrom = {
  ...
  ): Promise<DeprecatedERC721Contract> {
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/DeprecatedERC721.json",
        "utf8"
      );
