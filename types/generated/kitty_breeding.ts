...
export class KittyBreedingContract extends BaseContract {
  public supportsInterface = {
    async callAsync(
      _interfaceID: string,
      defaultBlock?: Web3.BlockParam
    ): Promise<boolean> {
      const self = this as KittyBreedingContract;
  ...
  static async deployed(
    ...
    return new KittyBreedingContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittyBreeding.json",
        "utf8"
      );
