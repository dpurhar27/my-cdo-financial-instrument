...
export class KittyAccessControlContract extends BaseContract {
  public cfoAddress = {
    async callAsync(defaultBlock?: Web3.BlockParam): Promise<string> {
      const self = this as KittyAccessControlContract;
  ...
  static async deployed(
  ...
  ): Promise<KittyAccessControlContract> {
    ...
    return new KittyAccessControlContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittyAccessControl.json",
        "utf8"
      );
