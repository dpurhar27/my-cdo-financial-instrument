...
export class TrancheTokenContract extends BaseContract {
  ...
  public mintTrancheToken = {
    async sendTransactionAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
        txData,
        self.mintTrancheToken.estimateGasAsync.bind(
          self,
          beneficiary,
          cdo,
          trancheLevel
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.mintTrancheToken,
        self.web3ContractInstance
      )(beneficiary, cdo, trancheLevel, txDataWithDefaults);
      return txHash;
    },
    async estimateGasAsync(
      beneficiary: string,
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.mintTrancheToken.estimateGas,
        self.web3ContractInstance
      )(beneficiary, cdo, trancheLevel, txDataWithDefaults);
      return gas;
    },
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/TrancheToken.json",
        "utf8"
      );
