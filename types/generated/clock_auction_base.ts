...
export class ClockAuctionBaseContract extends BaseContract {
  public ownerCut = {
    async callAsync(defaultBlock?: Web3.BlockParam): Promise<BigNumber> {
      const self = this as ClockAuctionBaseContract;
  ...
  static async deployed(
  ...
  ): Promise<ClockAuctionBaseContract> {
    ...
    return new ClockAuctionBaseContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/ClockAuctionBase.json",
        "utf8"
      );
