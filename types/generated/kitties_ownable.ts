...
export class KittiesOwnableContract extends BaseContract {
  public owner = {
    async callAsync(defaultBlock?: Web3.BlockParam): Promise<string> {
      const self = this as KittiesOwnableContract;
  ...
  static async deployed(
  ...
  ): Promise<KittiesOwnableContract> {
    ...
    return new KittiesOwnableContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittiesOwnable.json",
        "utf8"
      );
