...
export class KittiesERC721MetadataContract extends BaseContract {
  public getMetadata = {
    async callAsync(
      _tokenId: BigNumber,
      index_1: string,
      ...
      const self = this as KittiesERC721MetadataContract;
      const result = await promisify<[string[], BigNumber]>(
        self.web3ContractInstance.getMetadata.call,
  ...
  static async deployed(
  ...
  ): Promise<KittiesERC721MetadataContract> {
    ...
    return new KittiesERC721MetadataContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/KittiesERC721Metadata.json",
        "utf8"
      );
