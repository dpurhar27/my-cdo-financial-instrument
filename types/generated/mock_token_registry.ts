...
export class MockTokenRegistryContract extends BaseContract {
  ...
  public mockGetTokenAddressBySymbol = {
    async sendTransactionAsync(
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.mockGetTokenAddressBySymbol,
        self.web3ContractInstance
    ...
    async estimateGasAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.mockGetTokenAddressBySymbol.estimateGas,
        self.web3ContractInstance
      )(symbol, token, txDataWithDefaults);
      return gas;
    },
    getABIEncodedTransactionData(
      ...
      const self = this as MockTokenRegistryContract;
      const abiEncodedTransactionData = self.web3ContractInstance.mockGetTokenAddressBySymbol.getData();
      return abiEncodedTransactionData;
    }
  };
  public reset = {
  ...
  public mockGetTokenAddressByIndex = {
    async sendTransactionAsync(
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.mockGetTokenAddressByIndex,
        self.web3ContractInstance
      )(index, token, txDataWithDefaults);
      return txHash;
    },
    async estimateGasAsync(
      index: BigNumber,
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.mockGetTokenAddressByIndex.estimateGas,
        self.web3ContractInstance
      )(index, token, txDataWithDefaults);
      return gas;
    },
    getABIEncodedTransactionData(
      ...
      const self = this as MockTokenRegistryContract;
      const abiEncodedTransactionData = self.web3ContractInstance.mockGetTokenAddressByIndex.getData();
      return abiEncodedTransactionData;
    }
  };
  ...
  ): Promise<MockTokenRegistryContract> {
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/MockTokenRegistry.json",
        "utf8"
      );
