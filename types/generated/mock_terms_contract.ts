...
export class MockTermsContractContract extends BaseContract {
  ...
  public mockRegisterTermStartReturnValue = {
    async sendTransactionAsync(
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.mockRegisterTermStartReturnValue,
        self.web3ContractInstance
      )(agreementId, debtor, success, txDataWithDefaults);
      return txHash;
    },
    async estimateGasAsync(
      agreementId: string,
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
      const gas = await promisify<number>(
        self.web3ContractInstance.mockRegisterTermStartReturnValue.estimateGas,
        self.web3ContractInstance
      )(agreementId, debtor, success, txDataWithDefaults);
      return gas;
    },
    getABIEncodedTransactionData(
      ...
      const self = this as MockTermsContractContract;
      const abiEncodedTransactionData = self.web3ContractInstance.mockRegisterTermStartReturnValue.getData();
      return abiEncodedTransactionData;
    }
  };
  ...
  ): Promise<MockTermsContractContract> {
  ...
  private static async getArtifactsData(web3: Web3): Promise<any> {
    try {
      const artifact = await fs.readFile(
        "build/contracts/MockTermsContract.json",
        "utf8"
      );
